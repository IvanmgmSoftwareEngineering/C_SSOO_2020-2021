...
// EMPIEZA FUNCIONALIDAD PROGRAMA
		...
		printf("	       Para finalizar el programa pulse Ctrl+C\n");
		printf("-----------------------------------------------------------\n");
		pid_fork=fork();

		if(pid_fork<0){ // Error al hacer fork()
			...
			while(1){ // Así evitamos que el hijo termine
				...
				close(fd_tuberia_del_hijo_al_padre[1]);// A través de este Pipe con flujo de datos Padre<<----Hijo el hijo solo escribe, no lee. Cierro fd_thp[1].
				read(fd_tuberia_del_padre_al_hijo[0], buf_hijo_recepcion, 1024);
				printf("Hijo: buf_hijo_recepcion= %s\n", buf_hijo_recepcion);
				//Ahora transformamos caracter a caracter las minusculas en mayusculas
				...
				write(fd_tuberia_del_hijo_al_padre[0], buf_hijo_envio, 1024);

				//kill(getppid(),100);?? para despertar al padre del pause
			}
			...
			close(fd_tuberia_del_hijo_al_padre[0]);// A través de este Pipe con flujo de datos Padre<<----Hijo el padre solo lee, no escribe. Cierro fd_thp[0].
			while(fgets(buf_padre_envio, 1024, stdin) != NULL){
				...
				printf("Padre: buf_padre_envio= %s\n", buf_padre_envio);
				write(fd_tuberia_del_padre_al_hijo[1], buf_padre_envio, 1024);
				//wait() o pause()???
				read(fd_tuberia_del_hijo_al_padre[1], buf_padre_recepcion, 1024);
				printf("%s\n",buf_padre_recepcion);
				fflush(stdin);
			}
...
// IMPLEMENTACIONES FUNCIONES CUYAS CABECERAS ESTAN DEFINIDAS AL PRINCIPIO DE ESTE DOCUMENTO Y QUE SON UTILIZADAS DENTRO DE MAIN
	...
	void manejador_padre(int sig){
		...
		// ZONA DE FUNCIONALIDAD FUNCION
			...
			if(sig==SIGUSR1){//El hijo ya esta muerto, entonces podemos matar al padre
				...
				close(fd_tuberia_del_padre_al_hijo[1]);//Cerramos los Pipe's antes de abandonar el programa
				close(fd_tuberia_del_hijo_al_padre[1]);//Cerramos los Pipe's antes de abandonar el programa
				kill(pid_fork,SIGUSR2); //Enviamos señal al hijo para que muera
				wait(NULL); // esperamos a que el hijo muera como buen padre
				printf("El hijo termino\n");
				exit(0); // matamos al padre
			}else if (sig==100){
	...
	//IMPLEMENTACION FUNCION 2
		void manejador_hijo(int sig){
				...
				if(sig==SIGINT){// SIGINT=2 , Enviamos señal al padre y matamos al hijo
					...
					kill(getppid(), SIGUSR1); //Enviamos al Padre una señal para el sepa que debe terminar
					exit(0);
				}else if(sig==SIGUSR2){//el padre esta esperando a que el hijo muera para despues el morir
					close(fd_tuberia_del_padre_al_hijo[0]);//Cerramos los Pipe's antes de abandonar el programa
					close(fd_tuberia_del_hijo_al_padre[0]);//Cerramos los Pipe's antes de abandonar el programa
					exit(0);
