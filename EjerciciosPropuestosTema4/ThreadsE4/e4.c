 ...
 * 		Error n:
 *
 * Descripción: CENA DE LOS FILÓSOFOS con tratamiento de Interbloqueo utilizando CONDICIONES DE MUTEX
 *
 *
 */
//----------------------------------------------------------------------------
...
// ZONA DECLARACION VARIABLE GLOBALES (fuera de la función main())
	pthread_mutex_t vector_mutexs_palillos[5];// Variable de tipo MUTEX declarada de forma Global para que puede ser vista desde funciones externas que utiliza la hebra para su ejecución.
	pthread_cond_t vector_conds_mutex[5]; // Una CONDICIÓN de MUTEX por cada palillo
	char *estado_filosofos[5]; //pensando, comiendo, esperando
...
//-----------------------------------------------------------------------------------
		...
		signal(SIGINT,manejadorSenal);
			...
			pthread_cond_init(&vector_conds_mutex[0],NULL); //Inicializamos la CONDICION 0 de MUTEX
			pthread_cond_init(&vector_conds_mutex[1],NULL); //Inicializamos la CONDICION 1 de MUTEX
			pthread_cond_init(&vector_conds_mutex[2],NULL); //Inicializamos la CONDICION 2 de MUTEX
			pthread_cond_init(&vector_conds_mutex[3],NULL); //Inicializamos la CONDICION 3 de MUTEX
			pthread_cond_init(&vector_conds_mutex[4],NULL); //Inicializamos la CONDICION 4 de MUTEX
...
// IMPLEMENTACIONES FUNCIONES CUYAS CABECERAS ESTAN DEFINIDAS AL PRINCIPIO DE ESTE DOCUMENTO Y QUE SON UTILIZADAS DENTRO DE MAIN
	void *funcion_hebra_filosofo(int *arg_id_filosofo){
		...
		while(1){
			strcpy(estado_filosofos[id_filosofo],"pensando");
			printf("Filósofo %i pensando\n",id_filosofo);
			printf("Filósofo 0: %s, Filósofo 1: %s, Filósofo 2: %s, Filósofo 3: %s, Filósofo 4: %s \n",estado_filosofos[0],estado_filosofos[1],estado_filosofos[2],estado_filosofos[3],estado_filosofos[4]);
			sleep((rand() % 5)+1); // (rand() % (N-M+1))+M devuelve un número aleatorio decimal entre M=1 y N=5
			printf("Filósofo %i quiere comer\n",id_filosofo);
			printf("Filósofo 0: %s, Filósofo 1: %s, Filósofo 2: %s, Filósofo 3: %s, Filósofo 4: %s \n",estado_filosofos[0],estado_filosofos[1],estado_filosofos[2],estado_filosofos[3],estado_filosofos[4]);
				strcpy(estado_filosofos[id_filosofo],"esperando");
				...
				pthread_mutex_lock(&vector_mutexs_palillos[id_filosofo]);//El Filosofo i intenta coger el paillo que tiene a su izquierda
					strcpy(estado_filosofos[id_filosofo],"comiendo");
